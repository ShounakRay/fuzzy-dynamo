...
fuzz_target!(|data: &[u8]| {
    ...
    while i + 4 <= data.len() && ops.len() < 16 {
        let op = match data[i] % 4 {
            ...
            1 => {
                let op = TriOp::Remove { worker_id: data[i+1], index: data[i+2] };
                i += 3;
                op
            }
            2 => {
                let op = TriOp::Clear { worker_id: data[i+1] };
                i += 2;
                op
            }
            _ => {
                let op = TriOp::Query {
                    ...
                    s1: data[i+2],
                    s2: data[i+3],
                    early_exit: false, // Disable early_exit for fair comparison
                };
                i += 4;
                op
            }
    ...
    let mut positional_blocks = FxHashMap::default();
    ...
    for op in &ops {
        match op {
            TriOp::Store { worker_id, h0, h1, h2 } => {
                ...
                let _ = concurrent.apply_event(&mut concurrent_lookup, event.clone());
                let _ = positional.apply_event(&mut positional_blocks, event);
            ...
            TriOp::Query { s0, s1, s2, early_exit } => {
                let seq: Vec<LocalBlockHash> = [s0, s1, s2].iter()
                    .map(|&b| LocalBlockHash((*b % 8) as u64))
                    .collect();

                let r_radix = radix.find_matches(seq.clone(), *early_exit);
                let r_concurrent = concurrent.find_matches_impl(&seq, *early_exit);
                let r_positional = positional.find_matches(&seq, *early_exit);

                // RadixTree == ConcurrentRadixTree (exact match)
                assert_eq!(r_radix.scores, r_concurrent.scores,
                    "Radix vs Concurrent score mismatch. Query: {:?}", seq);

                // All three must agree on which workers are present
                let radix_workers: HashSet<_> = r_radix.scores.keys().collect();
                let positional_workers: HashSet<_> = r_positional.scores.keys().collect();
                assert_eq!(radix_workers, positional_workers,
                    "Worker set mismatch: Radix has {:?}, Positional has {:?}. Query: {:?}",
                    radix_workers, positional_workers, seq);

                // All three must agree on scores (same prefix depth)
                assert_eq!(r_radix.scores, r_positional.scores,
                    "Radix vs Positional score mismatch. Query: {:?}\n\
                     Radix:      {:?}\n\
                     Positional: {:?}",
                    seq, r_radix.scores, r_positional.scores);
            }
        }
    }
});
