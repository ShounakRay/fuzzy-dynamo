...
\begin{enumerate}
  \item \textbf{Chat template application.} The user's message array is converted into the specific prompt format the model was trained on. Dynamo uses \texttt{minijinja}, a Rust Jinja2 engine, to apply model-specific chat templates. Different models require wildly different formats---Llama uses \texttt{<|begin\_of\_text|>} markers, ChatGLM uses \texttt{[gMASK]<sop>}, and Mistral uses \texttt{[INST]} tags. The template is loaded from the model's configuration at startup.
...
\texttt{dynamo-async-openai} & Fork of \texttt{async-openai} with Dynamo extensions (BYOT---bring your own transport) \\
...
\midrule
\texttt{frontend} & Python frontend service that initializes the Rust HTTP server, configures model endpoints, and manages the serving pipeline lifecycle \\
\texttt{router} & Python router that configures and starts the Rust KV-aware router \\
\texttt{planner} & Auto-scaling planner: monitors latency/throughput metrics and decides when to scale prefill and decode worker pools \\
...
\texttt{docker-compose.yml} & Local dev deployment with etcd, NATS, and worker containers \\
\texttt{nats-server.conf} & NATS config (JetStream enabled, memory/file storage limits) \\
\texttt{observability} & Prometheus, Grafana, and OpenTelemetry for monitoring \\
\bottomrule
\end{tabular}
\end{center}
