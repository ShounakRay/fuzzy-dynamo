...
The \texttt{overlaps} field comes from the indexer's \texttt{find\_matches}. The fields \texttt{decode\_blocks} and \texttt{prefill\_tokens} come from the active sequence tracker, which monitors in-flight requests. The optional \texttt{allowed\_\allowbreak worker\_ids} enables Endpoint Priority Policies (EPP) to restrict routing to a subset of workers.
...
\begin{enumerate}
  \item The \texttt{Scheduling\-Request} arrives with \texttt{isl\_tokens} (input sequence length), pre-computed \texttt{overlaps} from the indexer, and per-worker \texttt{decode\_blocks} and \texttt{prefill\_tokens} from the active sequence tracker.
...
\begin{itemize}
  ...
  \item \textbf{Three indexer implementations} serve different concurrency needs: \texttt{Radix\-Tree} (single-threaded), \texttt{Concurrent\-Radix\-Tree} (multi-reader), and \texttt{Positional\-Indexer} (multi-threaded, flat hash map with jump search).
