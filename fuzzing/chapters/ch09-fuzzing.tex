...
The \texttt{fuzz\_triple\_differential} target is the most sophisticated harness in the campaign. It maintains three independent implementations of the KV-cache indexer---\texttt{Radix\-Tree}, \texttt{Concurrent\-Radix\-Tree}, and \texttt{Positional\-Indexer}---and feeds the same sequence of \texttt{Store}, \texttt{Remove}, \texttt{Clear}, and \texttt{Query} operations to all three. After each \texttt{Query}, it asserts that all three return identical overlap~scores:
...
The parser differential target (\texttt{fuzz\_differential}) feeds the same text to both the oneshot parser (\texttt{detect\_and\_parse\_reasoning}) and the streaming parser (\texttt{parse\_reasoning\_streaming\_\allowbreak incremental}), comparing their~outputs:
