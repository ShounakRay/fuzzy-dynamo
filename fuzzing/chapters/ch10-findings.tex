...
\item In \textbf{release mode} (how production runs), \texttt{24 + u64::MAX + 1} wraps to \texttt{25}. The \texttt{max\_message\_size} check passes because \texttt{25 < max\_size}. The codec then attempts to parse 25 bytes of data as a message whose header and body lengths claim to total $2^{64}$ bytes---producing garbage output from whatever happens to be in the buffer.
\end{enumerate}
...
\marginnote{\emph{Checked arithmetic:} Rust provides \texttt{checked\_add}, \texttt{checked\_mul}, etc., which return \texttt{None} on overflow instead of panicking or wrapping.}%
The lesson is clear: any Rust code that performs arithmetic on values from untrusted input---network data, configuration files, inter-process messages---should use checked arithmetic (\texttt{checked\_add}, \texttt{checked\_mul}) or enable overflow checks in the release profile:
